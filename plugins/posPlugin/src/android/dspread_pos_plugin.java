	...
	class MyPosListener extends CQPOSService {
		...
		public void onDoTradeResult(DoTradeResult arg0, Hashtable<String, String> arg1) {
			if (arg0 == DoTradeResult.NONE) {
				...
				callbackResult(PluginResult.Status.ERROR, "no_card_detected");
				...
				callbackResult(PluginResult.Status.ERROR, "card_inserted/NOT_ICC");
				...
				TRACE.w(arg0+":NFC2 "+content);
				// sendMsg(8003);
				Hashtable<String, String> h =  pos.getNFCBatchData();
				TRACE.w("nfc batchdata: \n"+h);
				content += "NFCbatchData: \n"+h.get("tlv");
				callbackKeepResult(PluginResult.Status.OK, "onRequestBatchData: "+content);
			} else if ((arg0 == DoTradeResult.NFC_DECLINED) ) {
				TRACE.d("transaction_declined");
				callbackKeepResult(PluginResult.Status.ERROR, "NFC_transaction_declined");
			}else if (arg0 == DoTradeResult.NO_RESPONSE) {
				TRACE.d("card_no_response");
				callbackKeepResult(PluginResult.Status.ERROR, "NFC_card_no_response");
			}
		}
